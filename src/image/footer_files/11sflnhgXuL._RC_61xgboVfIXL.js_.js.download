...
(!c||!p(c.element,c.minVisiblePercentForLeader||(b?.95:.5))||"visible"!==getComputedStyle(c.element).visibility)){var a=d.filter(function(a){return p(a.element,a.minVisiblePercentForLeader||(b?.95:.5))&&"visible"===getComputedStyle(a.element).visibility});m(0<a.length?a[0].element:l)}};a.on("scroll resize orientationChange",h);a.on("gw:mobile:registerHorizontalScrolling",function(a){"function"===typeof a.addEventListener&&a.addEventListener("scroll",h)});e.when("gw-desktop-herotator").execute(function(a){a.addObserver("animation_complete",
...
elect:m,subscribeToHorizontal:function(a){a&&e.when("A","ready").execute(function(b){b.trigger("gw:mobile:registerHorizontalScrolling",a)})}}})});
/* ******** */
////////////////////////////////////////////
      ...
      bindEvents: function () {
        ...
        self.$carousel.change(function(e) {
          ...
          if(self.defaultCssRightMargin === null && self.$shelf.children('li').first()) {
            ...
            self.defaultCssRightMargin = isNaN(firstMargin) ? 0 : firstMargin;
          }
